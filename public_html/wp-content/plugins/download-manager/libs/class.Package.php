...
class Package {
    ...
    function Prepare($ID = null, $template = null, $force = false)
    {
        ...
        $post_vars['link_label'] = $post_vars['link_label']?$post_vars['link_label']:__('Download Free Meditation Text','wpdmpro');
...
//        }
        ...
        if(isset($data['terms_lock']) && $data['terms_lock'] != 0 && (!function_exists('wpdmpp_effective_price') || wpdmpp_effective_price($post_vars['ID']) ==0)){
            ...
            $data['terms_title'] = !isset($data['terms_title']) || $data['terms_title'] == ''?__("Terms and Conditions", "wpdmpro"):$data['terms_title'];
            $data['terms_check_label'] = !isset($data['terms_check_label']) || $data['terms_check_label'] == ''?__("You Must Agree With Terms and Conditions to Download", "wpdmpro"):$data['terms_check_label'];
    ...
    private static function activeLocks($package, $params = array('embed'=>0, 'popstyle' => 'pop-over')){
        ...
        if($sociallock!=""){
            $data .= "<div class='panel panel-default text-center panel-package-locks'><div class='panel-heading'>".get_option('_wpdm_social_lock_panel_title', 'Like or Share to Download')."</div><div class='panel-body wpdm-social-locks text-center'><p>".get_option('_wpdm_social_lock_panel_desc', 'Please support us, use one of the buttons below to unlock the download link.')."</p>{$sociallock}</div></div>";
        }
        ...
        if ($lock === 'locked') {
            ...
            if ($embed == 1)
                ...
                $dataattrs = $popstyle == 'pop-over'? 'data-title="'.__('Download','wpdmpro').' ' . $package['title'] . '"' : 'data-toggle="modal" data-target="#pkg_' . $package['ID'] . "_" . $unqid . '"';
                $adata = '<a href="#pkg_' . $package['ID'] . "_" . $unqid . '" '.$dataattrs.' data-trigger="manual" onClick="ga(\'send\', \'event\', \'PDF\', \'download\', \'Meditation Text\', \'1\');" class="wid-but wpdm-download-link wpdm-download-locked ' . $popstyle . ' ' . $package['btnclass'] . '" >' . $package['link_label'] . '</a>';
    ...
    public static function fetchTemplate($template, $vars, $type = 'link')
    {
        ...
        $vars['fav_button_sm'] = self::favBtn($vars['ID'], array('size' => 'btn-sm', 'a2f_label' => "<i class='fa fa-heart'></i> &nbsp; ". __('Add to favourite', 'wpdmpro'), 'rff_label' => "<i class='fa fa-heart'></i> &nbsp; ". __('Remove from favourite', 'wpdmpro')));
        $vars['fav_button_ico_sm'] = self::favBtn($vars['ID'], array('size' => 'btn-sm', 'a2f_label' => "<i class='fa fa-heart'></i>", 'rff_label' => "<i class='fa fa-heart'></i>"));
    ...
    public static function favBtn($id, $atfb = array()){
        if(empty($atfb))
            $atfb = array('size' => '', 'a2f_label' => "<i class='fa fa-heart'></i> &nbsp; ". __('Add to favourite', 'wpdmpro'), 'rff_label' => "<i class='fa fa-heart'></i> &nbsp; ". __('Remove from favourite', 'wpdmpro'));
