...
//        ->Fetch('file-list.php');
            ...
            if (is_array($allfiles)) {
                ...
                foreach ($allfiles as $fileID => $sfile) {
                    ...
                    if ($swl) {
                        ...
                        $mp3 = end($mp3);
                        $mp3 = strtolower($mp3);
                        $play = $mp3 == 'mp3'?"<a rel='nofollow' class='btn btn-success btn-xs wpdm-btn-play song-{$file['ID']}-{$pc}' data-song-index='song-{$file['ID']}-{$pc}' id='song-{$file['ID']}-{$pc}' data-state='stop' href='#' data-player='audio-player-{$file['ID']}' data-song='" . $download_url . "&forceplay=1&ind=" . $ind . "'><i style='margin-top:0px' class='fa fa-play'></i></a>":"";
                        ...
                        if ($filePass != '' && $pwdlock)
                            $fhtml .= "<td width=150><button class='inddl btn btn-primary btn-sm' data-pid='{$file['ID']}' data-file='{$fileID}' rel='" . $permalink.$sap."wpdmdl={$file['ID']}" . "&ind=" . $ind . "' data-pass='#pass_{$file['ID']}_{$ind}'><i class='fa fa-download'></i>&nbsp;".$button_label."</button>&nbsp;{$individual_file_actions}</td></tr>";
                        else{
                            $ind_download_link = "<a rel='nofollow' class='btn btn-primary btn-xs' href='" . $download_url . "&ind=" . $ind . "'>".$button_label."</a>";
                            $ind_download_link = apply_filters("wpdm_single_file_download_link", $ind_download_link, $sfile, $download_url,  $file);
                            $fhtml .= "<td style='width: 150px;white-space: nowrap;'  class='text-right'>{$ind_download_link}{$play}&nbsp;{$individual_file_actions}</td></tr>";
                        }
                    } else {
                ...
                foreach ($dfiles as $ind => $sfile) {
                    ...
                    if ($idvdl == 1 && ($pwdlock || !$olock)) {
                        ...
                        if ($fileinfo[$sfile]['password'] != '' && $pwdlock)
                            $fhtml .= "<td width='110'  class='text-right'><input  onkeypress='jQuery(this).removeClass(\"input-error\");' size=10 type='password' value='' id='pass_{$file['ID']}_{$ind}' placeholder='".__('Password','wpdmpro')."' name='pass' class='form-control input-sm inddlps' /></td>";
                        if ($fileinfo[$sfile]['password'] != '' && $pwdlock)
                            $fhtml .= "<td width=150><button class='inddl btn btn-primary btn-sm' data-pid='{$file['ID']}' data-file='{$sfile}' rel='" . $download_url . "&ind=" . $ind . "' data-pass='#pass_{$file['ID']}_{$ind}'><i class='fa fa-download'></i> &nbsp;".$button_label."</button>&nbsp;{$individual_file_actions}</td></tr>";
                        else {
                            $ind_download_link = "<a rel='nofollow' class='btn btn-primary btn-xs' href='" . $download_url . "&ind=" . $ind . "'>".$button_label."</a>";
                            $ind_download_link = apply_filters("wpdm_single_file_download_link", $ind_download_link, $sfile, $download_url, $file);
                            $fhtml .= "<td style='width: 150px;white-space: nowrap;'  class='text-right'>{$ind_download_link}{$play}&nbsp;{$individual_file_actions}</td></tr>";
            ...
            if (is_array($allfiles)) {
                ...
                foreach ($allfiles as $fileID => $sfile) {
                    ...
                    if ($swl) {
                        ...
                        if ($filePass != '' && $pwdlock)
                            $fhtml .= "<span class='input-group-btn'><button class='inddl btn btn-primary btn-sm btn-block' file='{$sfile}' rel='" . $download_url . "&ind=" . $ind . "' pass='#pass_{$file['ID']}_{$ind}'><i class='fa fa-download'></i></button></span></div>";
                        else {
                            $ind_download_link = "<a rel='nofollow' class='btn btn-primary btn-sm btn-block' href='" . $download_url . "&ind=" . $ind . "'>".$button_label."</a>";
                            $ind_download_link = apply_filters("wpdm_single_file_download_link", $ind_download_link, $sfile, $download_url, $file);
    ...
    public static function Premium($file, $play_only = false)
    {
        ...
        if(!function_exists('wpdmpp_currency_sign')) return self::Table($file, $play_only);
        ...
        if (count($file['files']) > 0) {
            ...
            if (is_array($allfiles)) {
                ...
                foreach ($allfiles as $fileID => $sfile) {
                    ...
                    $pre_licenses = [
                        'statdard' => [ 'name' => 'Standard', 'use' => 1],
                        'extended' => [ 'name' => 'Extended', 'use' => 5],
                        'unlimited' => [ 'name' => 'Unlimited', 'use' => 99],
                    ];
                    $post_id = $file['ID'];
