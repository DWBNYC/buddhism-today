...
function plugin_update_email($plugin_name, $version, $update_url)
{
    ...
    if(!$sent) {
        $email = array(
            ...
            'default' => array('subject' => __($plugin_name . ': Update Available', 'wpdmpro'),
                ...
                'message' => 'New version available. Please update your copy.<br/><br/><table style="width: 100%" cellpadding="10px"><tr><td width="120px">Plugin Name:</td><td>' . $plugin_name . '</td></tr><tr><td width="120px">Version:</td><td>' . $version . '</td></tr><tr><td width="120px"></td><td><div style="padding-top: 10px;"><a class="btn" href="' . $update_url . '">Update Now</a></div></td></tr></table>'
            ));


        $template_file = "default.html";
        $template_data = file_get_contents(wpdm_tpl_path($template_file, WPDM_BASE_DIR . 'email-templates/'));
        $template['message'] = str_replace("[#message#]", stripslashes(wpautop($email['message'])), $template_data);

        $headers = "From: " . $template['from_name'] . " <" . $template['from_email'] . ">\r\nContent-type: text/html\r\n";

        wp_mail($admin_email, $template['subject'], $template['message'], $headers);
