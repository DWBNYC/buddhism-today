    ...
    if (file_exists($root.$dir)) {
        ...
        if (count($files) > 2) { /* The 2 accounts for . and .. */
            ...
            foreach ($files as $file) {
                if ($file != '.' && $file != '.htaccess' && $file != '..' && file_exists($root . $dir . $file) && is_dir($root . $dir . $file)) {
                    echo "<li class=\"directory collapsed\"><a id=\"" . uniqid() . "\" href=\"#\" rel=\"" . htmlentities($uroot . $dir . $file) . "/\">" . htmlentities($file) . "</a></li>";
                }
            }
            // All files
            foreach ($files as $file) {
                if ($file != '.' && $file != '..' && $file != '.htaccess' && file_exists($root . $dir . $file) && !is_dir($root . $dir . $file)) {
                    $ext = preg_replace('/^.*\./', '', $file);
                    echo "<li class=\"file ext_$ext\"><a id=\"" . uniqid() . "\" href=\"#\" rel=\"" . htmlentities($uroot . $dir . $file) . "\">" . htmlentities($file) . "</a></li>";
                }
            }
            echo "</ul>";
        }
        ...
        <script language="JavaScript">
            ...
            jQuery(function () {
                jQuery('#tree').fileTree({
                    root: '',
                    script: '<?php echo home_url('/?task=wpdm_dir_tree'); ?>',
                    expandSpeed: 1000,
                    collapseSpeed: 1000,
                    multiFolder: false
                }, function (file, id) {
